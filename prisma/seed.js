...
async function main() {
  ...
  const bottomCategory = await prisma.category.upsert({
    where: {
      name: "Bottom",
    },
    update: {},
  ...
  await Promise.all(
    productData.map(async (product, idx) => {
  ...
  await Promise.all(
    productDetailsData.map(async (productDetail, idx) => {
  ...
  await Promise.all(
    productGalleryData.map(async (productGallery, idx) => {
      await prisma.productGallery.upsert({
        where: { id: idx + 1 },
        update: {},
        create: {
          id: idx + 1,
  ...
  const adminData = [
    {
      username: "admin1",
      password: "$2b$10$QLkE0KoIcpoXwQpTw1o94OrwDBM0XCr5Q/C2XVNx6zoexCUAPn.eG",
      address: "123 Admin St",
    },
    {
      username: "admin2",
      password: "$2b$10$ItfEtkGdU7GoB4800m3JgO7mHZ2MK.YkAGnyHNFNTJ8JFM.m9gepm",
  ...
  await Promise.all(
    bankAccountData.map(async (bankAccount, idx) => {
